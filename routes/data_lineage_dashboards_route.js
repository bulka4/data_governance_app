...
router.get('/', checkAuthenticated, async (req, res) => {
    res.redirect('/data_lineage_dashboards/0')
})
...
router.get('/:id', checkAuthenticated, async (req, res) => {
    ...
    if (searchedQuery != '')
        docs = docs.filter(doc => {
            const searched_query_lowercase = searchedQuery.toLowerCase()
            if (doc.dataLineageName.toLowerCase().includes(searched_query_lowercase)) return true
            else {
                const nodes_matching_query = doc.nodes.filter(node => {
                    return node.value.toLowerCase().includes(searched_query_lowercase)
                })
    ...
    res.render('dataLineageDashboards', {
        docs: docs,
...
router.post('/search', async (req, res) => {
    ...
    if (searchedQuery != '')
        res.redirect(`/data_lineage_dashboards${actualUrl}?searchedQuery=${searchedQuery}`)
    else
        res.redirect(`/data_lineage_dashboards${actualUrl}`)
})
