...
async function createVisualization(dataLineageDocs, replace_nodes = true){
    ...
    const nodes = currentDataLineageDoc.nodes
    for (let node1 of nodes){
        for (let node2 of nodes){
            // if (node1.linkedTo.includes(node2.id) & node1.type != node2.type)
            if (node1.linkedTo.includes(node2.id))
...
function adjustVisualization(node){
    ...
    node
        ...
        .on('click', (event, data) => {
            fetch(`/data_lineage_dashboards/delete_node`, {
                method: 'post',
    ...
    node
        ...
        .on('click', async (event, nodeData) => {
            ...
            if (nodeData.type == 'table'){
                ...
                // add table description text to the popup window
                const modal_body = popUpWindow.querySelector('.modal-body')
                const modal_title = popUpWindow.querySelector('#modal_title')
                
                modal_body.textContent = nodeData.tableDescription
                modal_title.textContent = nodeData.value
            } 
            else if (nodeData.type == 'script') {
                popUpWindow = document.querySelector('#scriptAdditionalInfo')
                popUpWindow.classList.remove('hide')
                popUpWindow.classList.add('show')
                popUpWindow.style.overflowY = 'auto'
                popUpWindow.style.display = 'block'

                // add script text to the popup window
                const modal_body = popUpWindow.querySelector('.modal-body')
                const modal_title = popUpWindow.querySelector('#modal_title')

                modal_body.textContent = nodeData.script
                modal_title.textContent = nodeData.value
            }
        })
...
async function replaceNodes(
    ...
    `This function is setting up x and y coordinates of data lineage graph nodes such that they are all placed properly on a screen`

    // table nodes which are at the end of data lineage graphs. They are not used as input for any scripts. We will create seperate
    // data lineage graphs for each final table node.
    const final_tables = []
    data_lineage_doc.nodes.forEach(node => {
        if (node.linkedTo.length == 0) final_tables.push(node) 
    })

    const max_x_coordinate = data_lineage_graph_width
    // minimum y coordinate of nodes for one segment (segment consists of input tables nodes, script node and output table node)
    let min_y_coordinate
    // maximum y coordinate of a whole data lineage graph for one final table node. We need this so we can properly place a data lineage
    // graph for another final table node.
    let max_y_coordinate

    for (let final_table_node of final_tables){
        if (max_y_coordinate != undefined)
            min_y_coordinate = max_y_coordinate + 80
        else
            min_y_coordinate = 20

        // node level is a set of nodes with the same x coordinate
        ...
        for (let [i, level] of nodes_levels.entries()){
            ...
            let level_width
            try {
                level_width = level_nodes_values[0].length * 10
            } catch {
        ...
        // initial position of nodes. We set up x coordinates properly and for now we set y coordinate as min_y_coordinate. Later on
        // we will use the replaceSegments function to correct y corrdinates
        for (let [i, level] of nodes_levels.entries()){
        ...
        // Now we will be correcting y coordiantes for segments (segment consists of input tables nodes, script node and output table node)
        // We start with the final segment (with the final table node) and then other segments which are connected with the final segment
        // (segments are connected if an output table node from one segment is an input table node for another segment)
        // This is a recursive function which is replacing all connected segments.
        const input_tables_nodes = nodes_levels[2][0]
        const script_node = nodes_levels[1][0]
        const output_table_node = nodes_levels[0][0][0]
        ...
        // check what is maximum y coordinate of nodes created in this data lineage graph. We need to know this in order
        // to establish what will be minimum y coordinate for the next data lineage graph
        for (let level of nodes_levels){
            const level_nodes = level.flat()
            for (let node of level_nodes){
                if (node.y > max_y_coordinate | max_y_coordinate == undefined) max_y_coordinate = node.y
            }
        }
    }

    // save data about new nodes positions in the database
    await fetch(`/data_lineage_dashboards/save_data`, {
        method: 'post',
        body: JSON.stringify(data_lineage_doc.nodes),
        headers: new Headers({
            'Content-Type': 'application/json'
        })
    })
...
function replaceSegments(
    ...
    `This function is used in the replaceNodes function. It is replacing segments of a data lineage graph. A segment is a set of input tables nodes, 
    a script node and an output table node. This is a recursive function which is replacing all segments. It starts with a segment with nodes given by 
    the arguments: input_tables_nodes, script_node and output_table_node. Then it is replacing other segments which are connected to this segment 
    (segments are connected if an output table node of one segment is an input table node of another segment)
    
    output_table_level_nr argument is such a number that nodes_levels[output_table_level_nr] = output_table_node`

    // replace input tables nodes which are being created by some script
    let node_nr = 1
    for (let input_table_node of input_tables_nodes){
        ...
        let new_input_tables_min_y = (input_tables_min_y + node_nr * 40) - (new_input_tables_nodes.length - 1) * 40 / 2
        // make sure that input tables for the next iteration will not overlap with other nodes
        // nodes_levels[output_table_level_nr + 4].flat() are nodes from the same level as input tables for the next iteration
        nodes_levels[output_table_level_nr + 4].flat().forEach(node => {
            if (node.y > new_input_tables_min_y){
                new_input_tables_min_y = node.y + 40
            }
        })

        // make sure that the output table the next iteration will not overlap with other nodes
        // nodes_levels[output_table_level_nr + 2].flat() are nodes from the same level as the output table for the next iteration
        // new_output_table_y is the y coordinate of the output table for the next segment (next iteration)
        let new_output_table_y = (new_input_tables_min_y + (new_input_tables_nodes.length - 1) * 40 / 2)
        nodes_levels[output_table_level_nr + 2].flat().forEach(node => {
            if (Math.abs(node.y - new_output_table_y) <= 20){
                new_input_tables_min_y = node.y + 40
                new_output_table_y = (new_input_tables_min_y + (new_input_tables_nodes.length - 1) * 40 / 2)
            }
        })
    ...
    for (let input_table_node of input_tables_nodes){
        ...
        input_table_node.y = input_tables_min_y
    ...
    // check what is the biggest and lowest y coordinate of input tables nodes so we can correctly set up y coordinate
    // for the script and output tables nodes
    if (input_tables_nodes.length > 0){
        let input_tables_max_y = input_tables_nodes[0].y
        input_tables_min_y = input_tables_nodes[0].y
        input_tables_nodes.forEach(node => {
            if (node.y > input_tables_max_y) input_tables_max_y = node.y
            if (node.y < input_tables_min_y) input_tables_min_y = node.y
        })

        // replace the script node
        script_node.y = input_tables_max_y - (input_tables_max_y - input_tables_min_y) / 2
    } else {
        script_node.y = input_tables_min_y
    }
...
async function getData(){
    const response = await fetch('/data_lineage_dashboards/get_data', {
        method: 'get',
        headers: new Headers({
            'Content-Type': 'application/json'
        })
    ...
    async function dragended(event, d) {
        ...
        for (let node of displayedDocument.nodes){
            if (node._id != d._id & Math.abs(node.x - d.x) < 50 & Math.abs(node.y - d.y) < 20){
                ...
                if (d.linkedTo.includes(node.id)) d.linkedTo = d.linkedTo.filter((x) => {return x != node.id})
                else if (!node.linkedTo.includes(d.id)) d.linkedTo.push(node.id)
        ...
        // check if we should save data about nodes position. We do this if there is at least one node
        // which position was changed significantly, by more then value of the min_distance variable
        let save_data = false
        const min_distance = 20
        for (let node of displayedDocument.nodes){
            if (node._id == d._id){
                if (Math.abs(node.x - d.x) < min_distance & Math.abs(node.y - d.y) < min_distance){
                    continue
                } else {
                    save_data = true
                    break
        ...
        if (save_data){
            ...
            fetch(`/data_lineage_dashboards/save_data`, {
                method: 'post',
                ...
                createVisualization(dataLineageDocs, false)
            })
        }
...
// add event listener for closing a popup window with a sql script
const scriptAdditionalInfo = document.querySelector('#scriptAdditionalInfo')
scriptAdditionalInfo.querySelector('.close').addEventListener('click', x => {
    scriptAdditionalInfo.classList.add('hide')
    scriptAdditionalInfo.classList.remove('show')
})



// add event listener for closing a popup window with a table description
const tableAdditionalInfo = document.querySelector('#tableAdditionalInfo')
tableAdditionalInfo.querySelector('.close').addEventListener('click', x => {
    tableAdditionalInfo.classList.add('hide')
    tableAdditionalInfo.classList.remove('show')
})
